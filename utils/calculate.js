  ...
  return flightSegment?.flightList.reduce(
    (previous, current) => {
      ...
      if (flightSatisfiesPreferences(current, preferences)) {
        currentFare = current.fareList.find((fare) => fare.type === offerType);
        currentMiles = currentFare?.miles || Number.MAX_VALUE;
      }
...
const flightSatisfiesPreferences = (flight, preferences) => {
  ...
  return (
    (mappedCabinType === "all" || flight.cabin === mappedCabinType) &&
