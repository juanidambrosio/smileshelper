...
const flightSearchRepository = require('../db/repositories/flightSearchRepository');
...
const searchCityQuery = async (msg, match) => {
  ...
  const userId = msg.chat.username || msg.chat.id.toString();
  payload.preferences = match
    ? (await userRepository.getPreferences(userId)) || {}
    : {};
  ...
  if (!msg.isAlert) {
    await flightSearchRepository.createSearchFlight({
      id: userId,
...
const searchRoundTrip = async (msg, match) => {
  ...
  const userId = msg.chat.username || msg.chat.id.toString();
  payload.preferences = await userRepository.getPreferences(userId) || {};
  ...
  try {
    ...
    await flightSearchRepository.create({
      id: userId,
      origin: payload.origin,
      destination: payload.destination,
      departureDate: payload.departureDate,
      returnDate: payload.returnDate,
      price: bestFlights[0].departureFlight.price + bestFlights[0].returnFlight.price,
      searchType: "airport",
      smilesAndMoney: payload.preferences?.smilesAndMoney || false,
    });
